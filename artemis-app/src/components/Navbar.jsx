  ...
  return (
    <header className="w-full bg-[#839958] py-4 px-6 md:px-16 flex items-center justify-between shadow-md select-none">
      ...
      <div className="flex items-center gap-8">
        <div
          onClick={() => navigate('/')} // 3. Clic en el logo -> Ir a Inicio (/)
          className="text-white font-black text-3xl tracking-wider cursor-pointer"
          title="Ir a Inicio"
        >
          ÁRTEMIS
        </div>
        <nav className="hidden md:flex items-center gap-6">
          <button
            onClick={() => navigate('/')} // 4. Clic en "Recetas" -> Ir a Inicio (/)
            className="text-[#1D1D1D] hover:text-white font-semibold text-lg transition-colors"
          >
            Recetas
          </button>
        </nav>
      ...
      <div className="flex items-center gap-6">
        <button
          onClick={() => navigate('/subir')} // 5. Clic en Subir Receta -> Ir a /subir
        ...
        {isLoggedIn ? (
          /* =========================================================
             VISTA USUARIO CONECTADO (Marcador + Ícono de Perfil)
             ========================================================= */
          <div className="flex items-center gap-4 relative">
            {/* Botón: Recetas Guardadas (Marcador/Bookmark) -> Ir a /guardadas */}
            <button
              onClick={() => navigate('/guardadas')} // 6. Clic en el marcador -> Ir a /guardadas
            ...
            <div className="relative">
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border border-gray-200 py-2 z-50 flex flex-col text-left">
                  ...
                  {/* 7. Clic en Perfil -> Cierra el menú e va a /perfil */}
                  <button
                    onClick={() => {
